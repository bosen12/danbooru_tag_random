...
**這是暗房，不是工作臺。** 所以它不沿用 `web/tokens.css`——沿用就等於長得跟另外四間一樣，
而這是第五間，是遊戲。`game.css` 自己定一套。

色票從暗房的物理推出來：安全燈是琥珀色（`oklch(64% 0.17 48)`），相紙基底是冷的
（`oklch(9% 0.01 40)`）。安全燈當成真的光源畫在牆上，不只是拿來描邊。
...
沒有框、沒有底色，就是浮在相紙下緣的字。壓黑整個下半會吃掉衣服跟姿勢——
那正是題目要你看的東西——所以改用**離焦帶**：`backdrop-filter` 把托盤底下模糊並壓暗，
上緣用 mask 淡出。影像還在，只是沒對到焦。這也是放大機的語言。
