...
function renderEras() {
  ...
  if (note) {
    const era = exclusive ? settings.eras[0] : null;
...
function renderTray() {
  ...
  for (const tag of pinned) {
    ...
    btn.title = "點一下取消釘選";
    btn.addEventListener("click", () => {
      const next = applyClear(pinned, userBanned, tag);
      pinned = next.pinned;
      userBanned = next.userBanned;
      saveStore();
      renderCats();
...
function renderCats() {
  ...
  for (const sec of SECTIONS) {
    ...
    const items =
      sec.id === "quality"
        ? lex.data.quality.map((tag) => ({
            tag,
            section: "quality",
            group: "fixed",
            zh: (lex.data.zh || {})[tag],
          }))
        : lex.bySection[sec.id] || [];
    ...
    if (sec.id === "subject") {
      ...
      hint.textContent = `${vis.length} · ${sec.hint}`;
    }
    ...
    if (!wrap.querySelector(".sub") && sec.id !== "quality") {
      if (q) {
    ...
    if (wrap.querySelector(".sub") || sec.id === "quality") root.append(wrap);
  }
...
function makeTagBtn(item, sec, auto) {
  ...
  if (st === "banned") {
    ...
    btn.setAttribute(
      "aria-label",
      `${zh} (${item.tag}) ${mutexBan ? "與釘選互斥" : "已關閉"}`
    );
  }
  if (!fitsEra(item) && st !== "pinned") btn.dataset.offEra = "1";
  if (!fitsCast(item) && st !== "pinned") btn.dataset.offCast = "1";
  if (sec.id !== "subject" && !fitsHeat(item) && st === "pool") btn.dataset.offHeat = "1";
  btn.addEventListener("click", () => onTagClick(item.tag));
  return btn;
}
...
function bindUi() {
  ...
  $("q").addEventListener("input", () => {
    ...
    searchTimer = window.setTimeout(() => renderCats(), 120);
  });
