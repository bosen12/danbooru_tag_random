#!/usr/bin/env node
/** r31: same catalog as r24, new seed after pairing/prop fixes. */
...
writeFileSync(join(ROOT, "scripts/prompt_audit/r31.json"), JSON.stringify(rows));
const txt = [`r31 n=${rows.length} catalog=${catalog.length}\n`].concat(
  rows.map((r) => `#${r.id} ${r.label} ${r.mode}\nPIN ${r.pinned.join(", ")}\n${r.positive}\n`)
);
writeFileSync(join(ROOT, "scripts/prompt_audit/r31.txt"), txt.join("\n"));
console.log("wrote r31", rows.length, "catalog", catalog.length);
