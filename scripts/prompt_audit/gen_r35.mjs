#!/usr/bin/env node
/** r35: same catalog as r24, new seed after pairing/prop fixes. */
...
writeFileSync(join(ROOT, "scripts/prompt_audit/r35.json"), JSON.stringify(rows));
const txt = [`r35 n=${rows.length} catalog=${catalog.length}\n`].concat(
  rows.map((r) => `#${r.id} ${r.label} ${r.mode}\nPIN ${r.pinned.join(", ")}\n${r.positive}\n`)
);
writeFileSync(join(ROOT, "scripts/prompt_audit/r35.txt"), txt.join("\n"));
console.log("wrote r35", rows.length, "catalog", catalog.length);
