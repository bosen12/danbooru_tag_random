#!/usr/bin/env node
/** r24: 400 draws — left panel ops, eras, pin presets, catalog pins. */
...
writeFileSync(join(ROOT, "scripts/prompt_audit/r24.json"), JSON.stringify(rows));
const txt = [`r24 n=${rows.length} catalog=${catalog.length}\n`].concat(
  rows.map((r) => `#${r.id} ${r.label} ${r.mode}\nPIN ${r.pinned.join(", ")}\n${r.positive}\n`)
);
writeFileSync(join(ROOT, "scripts/prompt_audit/r24.txt"), txt.join("\n"));
console.log("wrote r24", rows.length, "catalog", catalog.length);
