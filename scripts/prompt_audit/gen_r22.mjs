#!/usr/bin/env node
/** r22: 400 draws — left panel + right-side tag pins. */
...
writeFileSync(join(ROOT, "scripts/prompt_audit/r22.json"), JSON.stringify(rows));
const txt = [`r22 n=${rows.length}\n`].concat(
  rows.map((r) => `#${r.id} ${r.label} ${r.mode}\nPIN ${r.pinned.join(", ")}\n${r.positive}\n`)
);
writeFileSync(join(ROOT, "scripts/prompt_audit/r22.txt"), txt.join("\n"));
console.log("wrote r22", rows.length, "catalog", catalog.length);
