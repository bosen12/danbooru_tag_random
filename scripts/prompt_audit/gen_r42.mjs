#!/usr/bin/env node
/** r42: same catalog as r24, seed after bed-support + pairing loosen. */
...
writeFileSync(join(ROOT, "scripts/prompt_audit/r42.json"), JSON.stringify(rows));
const txt = [`r42 n=${rows.length} catalog=${catalog.length}\n`].concat(
  rows.map((r) => `#${r.id} seed=${r.seed} ${r.label} ${r.mode}\nPIN ${r.pinned.join(", ")}\n${r.positive}\n`)
);
writeFileSync(join(ROOT, "scripts/prompt_audit/r42.txt"), txt.join("\n"));
console.log("wrote r42", rows.length, "catalog", catalog.length);
