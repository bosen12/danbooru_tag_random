#!/usr/bin/env node
/** r12: 1000 draws — left panel × eras × presets × pins. */
...
writeFileSync(join(ROOT, "scripts/prompt_audit/r12.json"), JSON.stringify(rows));
const txt = [`r12 n=${rows.length}\n`].concat(
  rows.map(
    (r) =>
      `#${r.id} ${r.label} mode=${r.mode} heats=${r.heats.join("+")} era=${r.era} cast=${r.cast} pin=[${r.pinned.join(", ")}]\n  cloth=[${r.clothing.join(", ")}]\n  pose=[${r.pose.join(", ")}]\n  env=[${r.env.join(", ")}]\n  POS: ${r.positive}\n`
  )
);
writeFileSync(join(ROOT, "scripts/prompt_audit/r12.txt"), txt.join("\n"));
