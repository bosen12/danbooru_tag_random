#!/usr/bin/env node
/** r33: same catalog as r24, new seed after pairing/prop fixes. */
...
writeFileSync(join(ROOT, "scripts/prompt_audit/r33.json"), JSON.stringify(rows));
const txt = [`r33 n=${rows.length} catalog=${catalog.length}\n`].concat(
  rows.map((r) => `#${r.id} ${r.label} ${r.mode}\nPIN ${r.pinned.join(", ")}\n${r.positive}\n`)
);
writeFileSync(join(ROOT, "scripts/prompt_audit/r33.txt"), txt.join("\n"));
console.log("wrote r33", rows.length, "catalog", catalog.length);
