#!/usr/bin/env node
/** r41: same catalog as r24, new seed after scene-cloth / kitchen / table fixes. */
...
writeFileSync(join(ROOT, "scripts/prompt_audit/r41.json"), JSON.stringify(rows));
const txt = [`r41 n=${rows.length} catalog=${catalog.length}\n`].concat(
  rows.map((r) => `#${r.id} ${r.label} ${r.mode}\nPIN ${r.pinned.join(", ")}\n${r.positive}\n`)
);
writeFileSync(join(ROOT, "scripts/prompt_audit/r41.txt"), txt.join("\n"));
console.log("wrote r41", rows.length, "catalog", catalog.length);
