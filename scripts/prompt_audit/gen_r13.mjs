#!/usr/bin/env node
/** r13: 600 draws — new presets, sports courts, rape, left panel. */
...
writeFileSync(join(ROOT, "scripts/prompt_audit/r18.json"), JSON.stringify(rows));
const txt = [`r18 n=${rows.length}\n`].concat(
  rows.map((r) => `#${r.id} ${r.label} ${r.mode} ${r.heats.join("+")}\n${r.positive}\n`)
);
writeFileSync(join(ROOT, "scripts/prompt_audit/r18.txt"), txt.join("\n"));
console.log("wrote r18", rows.length);
