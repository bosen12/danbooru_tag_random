...
- 正面的 `nsfw, explicit` 換成 `sfw, general`
- `nsfw / explicit / nude / nipples / pussy / penis / sex …` 共 16 個字加進**負面**
- 情色的字**整批抽不出來**（裸露、性行為、走光、脫衣、內衣、透視衣、臀部特寫、挑逗表情）
- **連你先前釘選的情色字也會被忽略** —— 釘選本來會繞過所有過濾，這裡是例外。
  你的釘選沒有被刪掉，開回色情模式就恢復

實抽 4320 張驗證：0 張帶 `nsfw/explicit`，0 個被擋的字漏出來，仍有 808 種字可用。

保證的範圍是「沒有性、沒有露點」。**不等於完全不性感** —— 網襪、短洋裝、
背向蹲姿這類仍然抽得到。要更保守的話，那幾個是下一個該擋的。
