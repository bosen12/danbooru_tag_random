...
按「抽並生圖」（暗房叫「曝光」，活字樓叫「付印」，抽籤棚叫「抽一籤」）時，**每一張都重新抽一套 POS**，然後排隊送給 Comfy（一次一張）。打開「這批同一個人」時，第 2 張起沿用第 1 張的髮／瞳／胸或男體型種族，衣、姿勢、場照抽；預設關。卡片上會先出現這張的中文 POS。抽樣過程顯示進度和 Comfy 的即時預覽，完成後換成成品、進度條拿掉。圖片格子一開始就依寬高鎖定比例，不會先拉很高再跳回來。
