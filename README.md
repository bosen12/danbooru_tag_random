...
| `start-web6.bat` | 墨池：排字匣的卡牌版，把字拖進合成池、引擎抽牌補齊；旁邊的疊印台（`/fuse.html`）一張牌疊一層墨 | http://127.0.0.1:8796 |
...
中間那張校樣立刻跟著變：風景牌鋪底、有人的那張用 multiply 疊上去、風格牌罩成濾鏡，
不花 ComfyUI 一秒鐘。

- **放牌**：字盒點一下，或拖到版上／紙上。同一格只留一張、時代對不上、附帶，都是 engine 的 `applyPin` 決定的，
  換下來的那張會說為什麼，旁邊有「換回」。
- **看關係**：版的右邊畫校對記號 —— 附帶（誰帶誰上來）、呼應（活動配地點）、相剋（晝夜、室內外…）。
- **試印**：四張同一批種子的 engine 抽牌，每疊一張就重抽一次（一次約 25ms），所以看得到加了這張之後引擎補的字怎麼變。
  每張牌底下四個小點是它在四張試印裡有沒有真的進 POS；都沒進就蓋斜線，點開會說原因。
  引擎補的牌以灰色影子排在套版裡，點一下可以收下。「換一批」換種子。
- **付印**：挑一張試印按付印才送 ComfyUI。印好的作品夾到上面的晾紙繩，點它可以回到那一版（牌、種子、挑的那張都回來）。
  ComfyUI 沒開、印壞了都不會動到版上的牌。
- 撤回（Z）、清版、當主版、把這一版放進墨池的合成池；規則和廢字簍跟墨池工作臺共用。
- 鍵盤：`/` 找牌、方向鍵在字盒裡走、Enter 放上、Delete 拿掉、`1`–`4` 換試印、`R` 換一批、`P` 付印。聲音可以關。
