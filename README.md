...
| `start-web5.bat` | 中控室：冷色儀表面板、電鈷藍訊號 | http://127.0.0.1:8794 |
| `start-game.bat` | 猜字棚：看圖猜 tag 的遊戲 | http://127.0.0.1:8791 |
| `start-zipu.bat` | 字鋪：揀字排版的牌組構築遊戲，成交就印成一張畫 | http://127.0.0.1:8795 |
...
「一次幾張」下面（墨池在「抽並生圖」下面、疊印台在「付印」下面、字鋪和猜字棚在開始畫面）有一組**生圖種子**：

- **隨機**（預設）：每張一顆新的種子，跟以前一樣。
- **固定**：填一個數字（0 到 9007199254740991），或按「固定」／骰子自動給一顆。打數字就直接切到固定。
  一次好幾張、無限抽，**每張都用同一顆**。

固定的只是送給 ComfyUI 的那顆（KSampler 的雜訊）；**抽牌照樣每張隨機**，所以固定之後是「同一種雜訊、不同的 POS」，
拿來比較內容換了之後圖差在哪。成品上的 `seed 數字` 可以點，點了就把那顆設成固定。
設定記在每個版面自己的瀏覽器裡；墨池和疊印台是同一個網站，所以共用。程式在 `web/seed-control.js`。

## 字鋪（牌組構築遊戲）

`start-zipu.bat`（或 `啟動-字鋪.bat`），開 <http://127.0.0.1:8795>。

每個字是一張牌，一行 POS 就是一手牌。客人上門下委託，你從字盒揀字排進手盒付印：
四種花色（長相、服裝、姿勢、場景）湊齊是全版，同類的字會撞字燒掉，東西配得上地方有呼應，
三張出自同一個古代是同代。分數到了就成交，那一單最好的一版會送去 ComfyUI 印成一張作品。
ComfyUI 沒開也能玩，作品會排隊等印刷機。

規則全部讀詞庫（互斥、時代、implies、情境表、運動組合、CLIP token 數），沒有另抄一份；
牌池只收全年齡的字。卡面插畫要先用自己的底模烘焙一次（不進版控，沒烘焙的牌用鉛字版卡面）：

```bash
python scripts/bake_card_art.py                  # 整本詞庫的卡面，約 1370 張，已有的跳過
python scripts/bake_card_art.py --rating general # 只烤全年齡（字鋪只用得到這些，約 1060 張）
python scripts/bake_card_art.py --extras         # 字鋪的道具、客人、背景
```

**第一次啟動不用自己打指令。** `start.bat`、`start-web6.bat`、`start-zipu.bat`（和對應的 `啟動-*.bat`）開網頁之前會先檢查卡面：
缺圖而且本機 ComfyUI 開著，就問要不要烤（1＝只烤全年齡、2＝全部分級、3＝這次不要、4＝以後都不要問），20 秒沒回答就選 1。
烤圖在另一個縮小的視窗跑，網頁照常開，烤好的牌重新整理就看得到；關掉那個視窗就停，已經烤好的會留著，下次啟動接著烤。
ComfyUI 沒開就只提醒一句，牌先用字形當封面。不想被問：選 4（會建一個 `.no-card-bake`），或啟動前 `set NO_CARD_BAKE=1`。
...
`node scripts/sim_zipu.mjs` 用機器人跑整局看難度。

## 墨池（排字匣的卡牌版）

`start-web6.bat`（或 `啟動-墨池.bat`），開 <http://127.0.0.1:8796>。

跟排字匣同一顆引擎、同一本詞庫、同一套分級與互斥，只是每個字都是一張有插畫的牌：

- **左邊字盒**：依花色（人、容、衣、姿、景、風）和小分類翻牌，搜尋中英文都可以。點一下或拖進合成池。
- **合成池**：放進去的牌每張圖都一定有（等於排字匣的釘選）；剩下的格子由引擎抽牌補齊。
  下面是規則（尺度、時代、誰、情境、尺寸），跟排字匣的左欄是同一組設定。
- **一次幾張、無限抽、同一個人、只抽牌、抽並生圖**；LoRA、底模、工作流跟排字匣共用同一個面板。
- **成品牆**：每張圖底下收著「牌 · N」，展開是這張用了哪些牌 —— 你放進池子的、引擎抽到的分開擺。
  成品上的牌拖回合成池＝釘選，拖到右下角的廢字簍＝封鎖。只抽牌沒有圖的那張預設展開。
- 滑鼠停在任何一張牌上，旁邊會浮出放大牌（插畫、英文 tag、分類、跟誰互斥）。

手機上長按 0.3 秒才開始拖，免得捲動時誤拖。封鎖、設定、最近 60 張成品存在瀏覽器裡（沒有期限，清掉網站資料才會不見）；合成池不存，重新整理就是空的。

從別台裝置（Mac 走 Tailscale、手機）生圖時網路斷一下不會丟圖：伺服器把那張留 30 秒（`GEN_REATTACH_SEC`），
墨池和疊印台會自動接回去；畫到一半重新整理頁面也會接回來。按「停」才真的取消（排隊中的也從 ComfyUI 佇列拿掉）。
其他房間照舊：連線一斷就中斷。
生圖時切到別的分頁，分頁標題會顯示進度（「40% · 墨池」），印好或印壞也寫在標題上，切回來就恢復。

### 疊印台（墨池旁邊的另一張桌子）

墨池右上角的「疊印台 →」，或直接開 <http://127.0.0.1:8796/fuse.html>。

一張牌是一層墨。中間的**卡池**照花色分成六列套版（罩色、姿勢、服裝、長相、人物、底色）：
你放的牌是實牌，同一列後面排著灰色的影子 —— 選中的那張試印裡引擎替你補的牌。整個卡池就是那一張圖的配方，
每一張都能點、能拖。右欄上面是那張試印印出來的成品（還沒印就是空白的紙），下面是四張試印和付印。

- **放牌**：字盒點一下，或拖進卡池。牌自己落進它花色那一列；同一格只留一張、時代對不上、附帶，
  都是 engine 的 `applyPin` 決定的，換下來的那張會說為什麼，旁邊有「換回」。拖回字盒就拿掉。
- **影子**：引擎補的牌。點一下看它是哪張試印補的、按「收下這張」；或直接拖進卡池。一列超過四張會收成「+N」，點開看全部。
- **看關係**：卡池右邊的留白畫校對記號 —— 附帶（誰帶誰上來）、呼應（活動配地點）、相剋（晝夜、室內外…），
  線沿著列與列之間的空隙走，不壓到牌。指著一張牌，它的線會亮起來、另一端那張描一圈。
- **試印**：四張同一批種子的 engine 抽牌，每疊一張就重抽一次（一次約 25ms），所以看得到加了這張之後引擎補的牌怎麼變。
  每張牌角落四個小點是它在四張試印裡有沒有真的進 POS；都沒進就蓋斜線，點開會說原因。
  換一張試印（點它，或按 `1`–`4`），卡池裡的影子就換成那一張補的。「換一批」換種子。
- **付印**：挑一張試印按付印才送 ComfyUI。印製中右欄上方看得到預覽和進度，印好的成品由上往下顯影，點它看大圖；
  同一張也夾到上面的晾紙繩，點它可以回到那一版（牌、種子、挑的那張都回來）。ComfyUI 沒開、印壞了都不會動到卡池裡的牌。
- 撤回（Z）、清版、把這一版放進墨池的合成池；規則和廢字簍跟墨池工作臺共用。
- 晾紙繩：滑鼠停在作品上會浮出一張大的；滑鼠滾輪上下滾就能左右捲，還有作品的那一頭會淡出。
- 成品在畫面上顯示的是 webp 小檔（約原圖的 1/12，伺服器請 ComfyUI 轉好後存在 `data/webp-cache/`），
  「開原圖」才拿原本的 PNG。新印的成品網址帶內容指紋（`&h=`），瀏覽器整年快取、重新整理不必再回伺服器問。
- 鍵盤：`/` 找牌、方向鍵在字盒和卡池裡走、Enter 放上／看選項、Delete 拿掉、`1`–`4` 換試印、`R` 換一批、`P` 付印。聲音可以關。
- 手機：卡池在最上面；捲到字盒時角落會留一顆「卡池」，點的牌會飛進去，點它回到卡池，也可以把牌拖到它上面。
- 跟著視窗大小走：寬螢幕上卡池的牌寬是照剩下的空間算的，六列盡量一次放進畫面（1920×1080 約 92px、2560×1440 約 138px，
  太小的視窗才捲）；每列的影子排到那一排放滿為止，其餘收成「+N」。字盒、右欄跟著視窗變寬，成品吃掉右欄剩下的高度。

卡池和試印的種子不存，重新整理就是空白的版；晾紙繩（最近 40 張）存在這台瀏覽器裡，點「回到這一版」能把當時的卡池叫回來。`node scripts/test_fuse.mjs` 用真的 engine 驗版上的規則。
...
按「抽並生圖」（暗房叫「曝光」，活字樓叫「付印」，抽籤棚叫「抽一籤」，中控室叫「執行」）時，**每一張都重新抽一套 POS**，然後排隊送給 Comfy（一次一張）。「一次幾張」旁邊的開關是「同一個人」：預設關。打開後第 1 張定臉，後面鎖髮／瞳／胸或男體型種族，衣場姿勢照抽。一次只有 1 張時去開，會自動改成 2 張。第 2 張起卡片左上角打「同 #1」。卡片上會先出現這張的中文 POS。抽樣過程顯示進度和 Comfy 的即時預覽，完成後換成成品、進度條拿掉。圖片格子一開始就依寬高鎖定比例，不會先拉很高再跳回來。
...
**目標**數量，不是保證值。釘選、骨架（髮長、頭髮顏色、瞳色、衣服、鏡頭、表情、場地／室內外／時間這類）和暗示帶進來的字都先計入，所以**實際數量可能超過目標**。

設成 0 或按「不補」＝**那一段整段不補**：骨架也不補，只留你釘的、釘的字帶上來的（implies／bind）和必抽。
四段都設 0 就只剩人物（1girl／solo）和畫質字。其他段照常補的時候，它們 implies 帶進來的字還是會出現在設 0 的那段
（例如泳衣帶上泳池）—— 那是那個字本身的一部分，不是補牌。所有版面（墨池、疊印台、導影台…）共用這一條。
...
篩選列最右邊的「字條／卡牌」切換字盒的樣子：卡牌模式每個字是一張有插畫的小牌，滑鼠停上去浮出放大牌；字條是原本一格一個字的樣子，最省空間。點法、權重、釘選封鎖兩種模式完全一樣，選了會記住。插畫要先烤過（見〈字鋪〉的 `bake_card_art.py`），沒烤到的字用字形當封面。插畫跟著左欄的尺度分級走：選全年齡時，敏感、色情字的牌蓋起來只露字形（圖不會下載），放大牌也一樣。
